...
public class IKey implements WritableComparable<IKey> {
	static {
		WritableComparator.define(IKey.class, new Comparator());
	}
	...
	public int compareTo(IKey another) {
		...
		cmp = this.column.compareTo(another.column);
	...
	public String toString() {
		return this.getPath().toString() + "," + this.getColumn().get();
	}
	...
	public static class KeyComparator extends WritableComparator {
		...
		public int compare(byte[] b1, int s1, int l1, byte[] b2, int s2, int l2) {
			try {
				...
				cmp = INT_COMPARATOR.compare(b1, s1 + firstL1 + 4, 4, b2, s2
						+ firstL2 + 4, 4);
	...
	public static byte[] serialize(IKey ikey) throws IOException {
	...
	public static IKey deserialize(byte[] data) throws IOException {
	...
	public static void main(String[] args) throws IOException {
		...
		first.setColumn(new IntWritable(2));

		IKey second = new IKey();
		second.setPath(new Text("/test/res1234"));
		second.setColumn(new IntWritable(0));
